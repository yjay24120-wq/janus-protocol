...
class RegimeSwitchingClassifier:
    ...
    def classify_market_regimes(market_matrix, lookback_window=30):
        ...
        # NOTE: previously this used np.percentile() over the WHOLE series, which means
        # every bar's regime label depended on volatility that hadn't happened yet
        # (look-ahead bias). We instead use an expanding (causal) quantile: the
        # threshold at bar i is only computed from bars <= i, so regimes are labeled
        # using information that would actually have been available at the time.
        # The only exception is the initial warm-up window (< lookback_window bars),
        # where there isn't enough history yet; those are back-filled from the first
        # valid causal estimate as a reasonable fallback.
        if len(vol_values) > 0:
            vol_series = pd.Series(vol_values)
            expanding_q33 = vol_series.expanding(min_periods=lookback_window).quantile(0.33).bfill()
            expanding_q66 = vol_series.expanding(min_periods=lookback_window).quantile(0.66).bfill()
            q33_arr = expanding_q33.values
            q66_arr = expanding_q66.values
        else:
            q33_arr = np.zeros_like(vol_values)
            q66_arr = np.zeros_like(vol_values)
        
        regimes = np.where(
            vol_values <= q33_arr, 0.0,          # Low Volatility state
            np.where(vol_values <= q66_arr, 1.0, # Normal Volatility state
                      2.0)                        # High Volatility state
        )
...
class Individual:
    ...
    def __init__(self, dna):
        ...
        self.profit_factor = 1.0  # Gross wins / gross losses, measured from backtest
        self.win_rate = 0.0       # Fraction of non-zero trades that were profitable
...
class MultiFactorChannelEvaluationNode:
    ...
    def evaluate_channel_strategy(individual, market_matrix, baseline_fee=1e-4):
        ...
        try:
            ...
            # NOTE: annualization factor was previously sqrt(252*78), which treats each
            # bar as one of 78 intraday (5-min) bars per trading day. But the data feed
            # here (yfinance default interval, and the synthetic generator) produces one
            # DAILY bar per row, so the correct annualization is sqrt(252). The old
            # factor inflated every Sharpe/Sortino by roughly sqrt(78) ≈ 8.8x.
            BARS_PER_YEAR = 252  # daily bars
            sharpe = (avg_ret / std_ret) * np.sqrt(BARS_PER_YEAR)
            ...
            sortino = (avg_ret / downside_std) * np.sqrt(BARS_PER_YEAR)
            ...
            annualized_pnl = avg_ret * BARS_PER_YEAR
            ...
            # Real profit factor (gross wins / gross losses) and win rate, measured
            # directly from this backtest's trades — used later for Kelly sizing
            # instead of conflating it with the Calmar ratio (see CapitalAllocationEngine).
            gains = strategy_returns[strategy_returns > 0]
            losses = strategy_returns[strategy_returns < 0]
            gross_win = float(np.sum(gains))
            gross_loss = float(-np.sum(losses))
            if gross_loss > 1e-12:
                individual.profit_factor = gross_win / gross_loss
            else:
                individual.profit_factor = 5.0 if gross_win > 0 else 1.0
            nonzero_trades = np.count_nonzero(strategy_returns)
            individual.win_rate = (len(gains) / nonzero_trades) if nonzero_trades > 0 else 0.0
...
class WalkForwardValidationEngine:
    ...
    def validate_strategy_robustness(self, elite_individual, market_matrix):
        ...
        # NOTE: previously this only required each split to have > 50 rows, regardless
        # of the strategy's actual warm-up requirement (slow_window = lookback * 4,
        # which can be up to 240 rows). Folds shorter than that silently produced no
        # trading signal and fell through to the 999.0 fallback, quietly polluting
        # the OOS degradation ratio. We now size the minimum fold length off the
        # individual's own DNA.
        lookback = max(5, int(round(elite_individual.dna[1])))
        min_required_rows = (lookback * 4) + 10
        ...
        for i in range(self.n_splits):
            ...
            if len(train_f) > min_required_rows and len(test_f) > min_required_rows:
...
class GeneticPersistenceLayer:
    ...
    def _init_schema(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            ''')
            # Migration for DBs created before profit_factor/win_rate existed.
            for col, coltype in (("profit_factor", "REAL"), ("win_rate", "REAL")):
                try:
                    conn.execute(f'ALTER TABLE evolved_channel_elites ADD COLUMN {col} {coltype}')
    ...
    def persist_elite(self, elite, oos_degradation=1.0):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute('''
                INSERT INTO evolved_channel_elites (sharpe_ratio, max_drawdown, tracking_jitter, oos_degradation, sortino, calmar, var_95, pareto_rank, dna_vector, profit_factor, win_rate)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (sharpe, elite.max_dd, elite.tracking_jitter, oos_degradation, elite.sortino, elite.calmar, elite.var_95, elite.pareto_rank, json.dumps(elite.dna), elite.profit_factor, elite.win_rate))
            conn.commit()
...
Channel Weight      : {dna_vector[3]:.4f}
            ...
            # Previously this passed the Calmar ratio in as if it were a profit factor
            # (a different metric on a different scale, and often negative), with a
            # max(1.1, ...) floor that masked bad strategies. We now use the actual
            # measured win rate and profit factor from the backtest, so a losing
            # strategy correctly produces a low/zero suggested allocation.
            row_profit_factor = selected_row['profit_factor'] if pd.notna(selected_row['profit_factor']) else 1.0
            row_win_rate = selected_row['win_rate'] if pd.notna(selected_row['win_rate']) else 0.0
            computed_kelly = CapitalAllocationEngine.compute_kelly_fraction(row_win_rate, row_profit_factor)
            st.markdown(f"**Win Rate:** `{row_win_rate*100.0:.1f}%`  |  **Profit Factor:** `{row_profit_factor:.2f}`")
